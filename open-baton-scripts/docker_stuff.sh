...
docker run -v /var/run/docker.sock:/var/run/docker.sock -d -h openbaton-rabbitmq -p 8080:8080 -p 5672:5672 -p 15672:15672 -p 8443:8443 -e RABBITMQ_BROKERIP=192.168.178.66 neoklosch/open_baton_docker_test

# build Open Baton project
./gradlew clean googleJavaFormat build

# copy jar to Open Baton in docker
docker cp ./build/libs/open_baton_docker_plugin-1.0-SNAPSHOT.jar elegant_morse:/usr/lib/openbaton/plugins/vim-drivers/openbaton-plugin-vimdriver-docker-1.0-SNAPSHOT.jar
